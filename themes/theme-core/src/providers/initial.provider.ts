...
import { ThemeCoreService } from '../services/theme-core.service'
import { RoutesService as AbpRoutesService, AbstractNavTreeService } from '@abp/ng.core';
...
export function configureInitial(store: Store, actions$: Actions, wrapRoutesService: WrapRoutesService) {
  return () => {
    initial(store, actions$, wrapRoutesService);
  };
}

function initial(store: Store, actions$: Actions, wrapRoutesService: WrapRoutesService) {

...
export function getEditModalKeys(options: { collection?: string }) {
  const editModalKey = options.collection
    ? `${options.collection}EditModal`
    : 'editModal';

  const updateEditModalKey = options.collection
    ? `update${capitalize(options.collection)}EditModal`
    : 'updateEditModal';
  const saveEditModalKey = options.collection
    ? `save${capitalize(options.collection)}EditModal`
    : 'saveEditModal';
  const fetchSelectedKey = options.collection
    ? `fetch${capitalize(options.collection)}Selected`
    : 'fetchSelected';
  const openEditModalKey = options.collection
    ? `open${capitalize(options.collection)}EditModal`
    : 'openEditModal';
...
export type NamedEditModalMethods<E, Collection extends string> = {
  [K in Collection as `update${Capitalize<K>}EditModal`]: (
  ...
  [K in Collection as `open${Capitalize<K>}EditModal`]: (
    id: string | null
  ) => void;
};
...
export function withEditModal<E, U, C, Collection extends string>(options: {
  ...
  EmptyFeatureResult,
  {
    ...
    props: NamedEditModalSignals<Collection>;
    methods: NamedEditModalMethods<E, Collection>;
  }
>;
export function withEditModal<E, U, C>(options: {
  extensionsIdentifier?: string;
  get: (id: string) => Observable<E>;
  update: (id: string, data: U) => Observable<E>;
  create: (data: C) => Observable<E>;
...
export function withEditModal<E, U, C, Collection extends string>(options: {
  ...
  return signalStoreFeature(
      ...
      (
            ...
            switchMap(() => {
              if (store[editModalKey]().selectedId === null) {
                ...
                return runInInjectionContext(injector, () =>
                  get(store[editModalKey]().selectedId || '').pipe(
                    tap((data) => {
                      patchState(
                        store,
                        setEditModal(options, { selected: data })
                      );
                    })
                  )
                );
        ...
        return {
                  ...
                  return (
                    ...
                    tapResponse({
                      next: () => {
                        patchState(
                          store,
                          setEditModal(options, { visible: false })
                        );
          ...
          [openEditModalKey]: (id: string | null) => {
            patchState(
              store,
              setEditModal(options, { selectedId: id, visible: true })
            );
            _fetchSelected();
          },
        };
    ...
    withComputed((store: any, injector = inject(Injector)) => {
      const getInjector = (id: string | null) =>
        id === null
          ? injector
          : Injector.create({
      ...
      return {
        [formKey]: computed(() => {
          const form = generateFormFromProps(
            new FormPropData(
              getInjector(extensionsIdentifier || null),
              store[editModalKey].selected()
            )
          );
          return form;

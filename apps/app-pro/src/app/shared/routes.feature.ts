...
export function getRoutesKeys(options: { collection?: string }) {
  const routesKey = options.collection ? `${options.collection}Routes` : 'routes';

  const getRoutesServiceKey = options.collection ? `get${capitalize(options.collection)}RoutesService` : 'getRoutesService';
  const addRoutesKey = options.collection ? `add${capitalize(options.collection)}Routes` : 'addRoutes';
  const hookRouterKey = options.collection ? `hook${capitalize(options.collection)}Router` : 'hookRouter';
...
export type NamedRoutesMethods<Collection extends string> = {
  [K in Collection as `get${Capitalize<K>}RoutesService`]: () => RoutesService;
    ...
    [K in Collection as `add${Capitalize<K>}Routes`]: (items: ABP.Route[]) => ABP.Route[];
  } & {
...
export function withRoutes<Collection extends string>(
  options: {
...
export function withRoutes<Collection extends string>(
  ...
  const { routesKey, addRoutesKey, getRoutesServiceKey, hookRouterKey } = getRoutesKeys(options);
  return signalStoreFeature(
    ...
    withMethods((store, injector = inject(Injector)) => {
      ...
      return {
        ...
        [hookRouterKey]: rxMethod<ActivatedRoute>(pipe(
                ...
                .pipe(
                  ...
                  tap(() => {
                    const isMatch = routesService?.flat.some(r => {
                      return router?.isActive(router.createUrlTree([r.path], {
                        relativeTo: activatedRoute
                      }), {
                        paths: 'subset', queryParams: 'subset', fragment: 'ignored', matrixParams: 'ignored'
                      })
                    });
                    patchState(store, { Route: { IsMatch: isMatch } });
                  })
                )
            })
          })

...
import { ChangeType, Tree, applyChangesToString } from '@nx/devkit';
...
export function addEntityPropToDefault(
  ...
  const PROPS_SELECTOR = `VariableDeclaration:has(Identifier[name=${variable}])  PropertyAssignment:has(Identifier[name=name])`;

  const entityPropNodes = tsquery(ast, PROPS_SELECTOR, {
    visitAllChildren: true,
  });

  if (entityPropNodes.length === 0) return;

  const _ignoreProperties = [
    ...defaultIgnoreProperties.map((x) => camel(x)),
    ...entityPropNodes.map((x) =>
      (x as ts.PropertyAssignment).initializer.getText().replace(/'/g, '')
    ),
    ...type.properties
      .filter(
        (x) => Object.keys(typeMap).find((y) => y === x.type) === undefined
      )
      .map((x) => camel(x.name)),
  ];

  const insertPropertiesTexts = type.properties
    .filter(
      (x) => _ignoreProperties.findIndex((y) => y === camel(x.name)) === -1
    )
    .map((x) => {
      switch (variable) {
        ...
        case Variable.EDIT_FORM_PROPS:
          ...
          displayName: \`\${resourceName}::${x.name}\`,
          defaultValue: ${
            defaultMap[x.type] ? 'false' : 'undefined'
          },            
          validators: () => [${x.isRequired ? 'Validators.required' : ''}],
        }`;
      }
  ...
  const PROPS_ARRAY = `VariableDeclaration:has(Identifier[name=${variable}])  ArrayLiteralExpression:has(Identifier[name=name])`;

  const propArray = tsquery(ast, PROPS_ARRAY, {
    visitAllChildren: true,
  });

  if (insertPropertiesTexts.length === 0) return;

  const insertIndex = propArray.reverse()[0].getEnd() - 1;

  const prefix = contents.slice(0, insertIndex).trim().endsWith(',')
    ? ''
    : ', ';

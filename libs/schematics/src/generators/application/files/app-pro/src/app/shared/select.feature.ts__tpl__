...
export function getSelectKeys(options: { collection?: string }) {
    ...
    const updateItemsKey = options.collection ? `update${capitalize(options.collection)}Items` : 'items';
    const clearItemsKey = options.collection ? `clear${capitalize(options.collection)}Items` : 'items';
    const loadMoreKey = options.collection ? `loadMore${capitalize(options.collection)}` : 'loadMore';
    const searchKey = options.collection ? `search${capitalize(options.collection)}` : 'search';
...
export function withSelect<E, I, Collection extends string>(
    options: {
...
>;
export function withSelect<E, I>(
    options: {
...
export function withSelect<E, I, Collection extends string>(
    ...
    const { datasKey, loadingKey, getListServiceKey, queryKey, updateQueryKey, hookQueryKey } = getListServiceKeys(options);
    const { itemsKey, updateItemsKey, clearItemsKey, loadMoreKey, searchKey } = getSelectKeys(options);
        ...
        withMethods((store: any) => {
            return {
                [updateItemsKey]() {
                    ...
                    const allItems = [...store[itemsKey](), ...store[datasKey]().items || []];
                    const allItemIds = allItems.map(item => item.id);

                    const filteredSelectedItems = selectedItems.filter(item => !allItemIds.includes(item.id));

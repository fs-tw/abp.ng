...
export function getSelectedKeys(options: { collection?: string }) {
  const selectedKey = options.collection ? `${options.collection}Selected` : 'selected';

  const updateSelectedKey = options.collection ? `update${capitalize(options.collection)}Selected` : 'updateSelected';
...
export type NamedSelectedMethods<E, Collection extends string> =
  {
    [K in Collection as `update${Capitalize<K>}Selected`]: (partialSelected: Partial<Selected<E>>) => void;
  }
...
export function withSelected<E, Collection extends string>(
  options: {
...
>;
export function withSelected<E>(
  options: {
    get: (id: string) => Observable<E>
  }
): SignalStoreFeature<

...
export async function preGenerateFiles(
    ...
    options: {
        rootNames: {
            fileName: string; name: string; className: string; propertyName: string; constantName: string;
        };
        storeNames: { fileName: string; name: string; className: string; propertyName: string; constantName: string; };
        componentType: ComponentKind; project: string; prefix: string; pageWrap: boolean; navStyle: string; name: string;
    ...
    switch (options.componentType) {
        case 'routes':
            if (options.path.trim().length !== 0)
                throw new Error('routes of ComponentType is not allowed to in sub directory, please use other ComponentType instead.');
            break;
        case 'entity':
            break;
        case 'form':
            if (options.path.trim().length === 0)
                throw new Error('form of ComponentType is not allowed to in root directory, please use routes or tabs ComponentType first.');
            break;
        case 'modal':
            if (options.path.trim().length === 0)
                throw new Error('form of ComponentType is not allowed to in root directory, please use routes or tabs ComponentType first.');
